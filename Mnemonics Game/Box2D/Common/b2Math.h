...
/// infinity.
inline bool b2IsValid(float32 x) { return isfinite(x); }
...
struct b2Vec2
{
	...
	/// Get the skew vector such that dot(skew_vec, other) == cross(vec,
	/// other)
...
template <typename T> inline T b2Clamp(T a, T low, T high)
{
...
/// by a SWAR algorithm that recursively "folds" the upper bits into the lower
/// bits. This process yields a bit vector with the same most significant 1 as
/// x, but all 1's below it. Adding 1 to that value yields the next largest
