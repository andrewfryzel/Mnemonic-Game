...
class b2World
{
	...
	/// Destruct the world. All physics entities are destroyed and all heap
	/// memory is released.
	~b2World();
	...
	/// collision. Otherwise the default filter is used (b2_defaultFilter).
	...
	/// Register a routine for debug drawing. The debug draw functions are
	/// called inside with b2World::DrawDebugData method. The debug draw
	/// object is owned by you and must remain in scope.
	...
	/// Manually clear the force buffer on all bodies. By default, forces
	/// are cleared automatically after each call to Step. The default
	/// behavior is modified by calling SetAutoClearForces. The purpose of
	/// this function is to support sub-stepping. Sub-stepping is often used
	/// to maintain a fixed sized time step under a variable frame-rate.
	/// When you perform sub-stepping you will disable auto clearing of
	/// forces and instead call ClearForces after all sub-steps are complete
	/// in one pass of your game loop.
	/// @see SetAutoClearForces
	void ClearForces();

	/// Call this to draw shapes and other debug draw data. This is
	...
	/// n-points. The ray-cast ignores shapes that contain the starting
	/// point.
	/// @param callback a user implemented callback class.
	...
	/// Get the world body list. With the returned body, use b2Body::GetNext
	/// to get the next body in the world list. A nullptr body indicates the
	/// end of the list.
	...
	/// b2Joint::GetNext to get the next joint in the world list. A nullptr
	...
	/// b2Contact::GetNext to get the next contact in the world list. A
	/// nullptr contact indicates the end of the list.
	/// @return the head of the world contact list.
	/// @warning contacts are created and destroyed in the middle of a time
	/// step. Use b2ContactListener to avoid missing contacts.
	...
	/// Get the flag that controls automatic clearing of forces after each
	/// time step.
