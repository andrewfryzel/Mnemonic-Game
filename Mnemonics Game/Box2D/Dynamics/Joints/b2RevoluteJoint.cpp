...
void b2RevoluteJoint::InitVelocityConstraints(const b2SolverData &data)
{
	...
	//     r1x*iA+r2x*iB] [          -r1y*iA-r2y*iB, r1x*iA+r2x*iB, iA+iB]
...
void b2RevoluteJoint::SolveVelocityConstraints(const b2SolverData &data)
{
	    ...
	    fixedRotation == false)
	{
		...
		else if (m_limitState == e_atLowerLimit)
		{
			...
			{
				b2Vec2 rhs =
				    -Cdot1 + m_impulse.z * b2Vec2(m_mass.ez.x,
								  m_mass.ez.y);
...
bool b2RevoluteJoint::SolvePositionConstraints(const b2SolverData &data)
{
	    ...
	    fixedRotation == false)
	{
		...
		if (m_limitState == e_equalLimits)
		{
			...
			float32 C = b2Clamp(angle - m_lowerAngle,
		...
		else if (m_limitState == e_atLowerLimit)
		{
			...
			// Prevent large angular corrections and allow some
			// slop.
			C = b2Clamp(C + b2_angularSlop,
				    -b2_maxAngularCorrection, 0.0f);
			limitImpulse = -m_motorMass * C;
		}
		else if (m_limitState == e_atUpperLimit)
		{
			...
			// Prevent large angular corrections and allow some
			// slop.
			C = b2Clamp(C - b2_angularSlop, 0.0f,
				    b2_maxAngularCorrection);
			limitImpulse = -m_motorMass * C;
		}
