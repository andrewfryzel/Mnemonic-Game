...
b2GearJoint::b2GearJoint(const b2GearJointDef *def) : b2Joint(def)
{
	...
	// TODO_ERIN there might be some problem with the joint edges in
	// b2Joint.
	...
	else
	{
		...
		b2Vec2 pB = b2MulT(xfD.q, b2Mul(xfB.q, m_localAnchorB) +
					      (xfB.p - xfD.p));
...
void b2GearJoint::InitVelocityConstraints(const b2SolverData &data)
{
	...
	else
	{
		...
		m_JwA = b2Cross(rA, u);
		m_mass +=
		    m_mC + m_mA + m_iC * m_JwC * m_JwC + m_iA * m_JwA * m_JwA;
	}
	...
	else
	{
		...
		m_mass += m_ratio * m_ratio * (m_mD + m_mB) +
			  m_iD * m_JwD * m_JwD + m_iB * m_JwB * m_JwB;
	}
