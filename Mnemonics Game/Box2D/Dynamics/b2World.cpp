	...
	if (def->collideConnected == false)
	{
		...
		while (edge)
		{
			if (edge->other == bodyA)
			{
				...
				// time step (where either body is awake).
...
void b2World::Solve(const b2TimeStep &step)
{
	...
	b2Island island(m_bodyCount, m_contactManager.m_contactCount,
			m_jointCount, &m_stackAllocator,
	...
	b2Body **stack =
	    (b2Body **)m_stackAllocator.Allocate(stackSize * sizeof(b2Body *));
	for (b2Body *seed = m_bodyList; seed; seed = seed->m_next)
	{
		...
		while (stackCount > 0)
		{
			...
			for (b2ContactEdge *ce = b->m_contactList; ce;
			     ce = ce->next)
			{
				...
				// Was the other body already added to this
				// island?
			...
			for (b2JointEdge *je = b->m_jointList; je;
			     je = je->next)
			{
				...
				// Don't simulate joints connected to inactive
				// bodies.
	...
	for (;;)
	{
		     ...
		     c = c->m_next)
		{
			...
			else
			{
				...
				b2Assert(typeA == b2_dynamicBody ||
					 typeB == b2_dynamicBody);

				bool activeA =
				    bA->IsAwake() && typeA != b2_staticBody;
				bool activeB =
				    bB->IsAwake() && typeB != b2_staticBody;
				...
				bool collideA =
				    bA->IsBullet() || typeA != b2_dynamicBody;
				bool collideB =
				    bB->IsBullet() || typeB != b2_dynamicBody;
				...
				// Compute the time of impact in interval [0,
				// minTOI]
				b2TOIInput input;
				...
				// Beta is the fraction of the remaining portion
				// of the .
				float32 beta = output.t;
				if (output.state == b2TOIOutput::e_touching)
				{
					alpha = b2Min(alpha0 + (1.0f - alpha0) *
								   beta,
						      1.0f);
				}
				else
		...
		if (minContact == nullptr ||
		    1.0f - 10.0f * b2_epsilon < minAlpha)
		{
			...
			if (body->m_type == b2_dynamicBody)
			{
				     ...
				     ce; ce = ce->next)
				{
					if (island.m_bodyCount ==
					    island.m_bodyCapacity)
					{
						break;
					}

					if (island.m_contactCount ==
					    island.m_contactCapacity)
					{
						break;
					}
					...
					// Has this contact already been added
					// to the island?
					if (contact->m_flags &
					...
					// Only add static, kinematic, or bullet
					// bodies.
					b2Body *other = ce->other;
					...
					// Skip sensors.
					bool sensorA =
					    contact->m_fixtureA->m_isSensor;
					bool sensorB =
					    contact->m_fixtureB->m_isSensor;
					if (sensorA || sensorB)
					{
						continue;
					}

					// Tentatively advance the body to the
					// TOI.
					b2Sweep backup = other->m_sweep;
					if ((other->m_flags &
					     b2Body::e_islandFlag) == 0)
					{
					...
					// Has the other body already been added
					// to the island?
					if (other->m_flags &
					    b2Body::e_islandFlag)
					{
		...
		for (int32 i = 0; i < island.m_bodyCount; ++i)
		{
			...
			for (b2ContactEdge *ce = body->m_contactList; ce;
			     ce = ce->next)
			{
				ce->contact->m_flags &=
				    ~(b2Contact::e_toiFlag |
...
struct b2WorldQueryWrapper
{
	bool QueryCallback(int32 proxyId)
	{
		b2FixtureProxy *proxy =
		    (b2FixtureProxy *)broadPhase->GetUserData(proxyId);
...
struct b2WorldRayCastWrapper
{
	float32 RayCastCallback(const b2RayCastInput &input, int32 proxyId)
	{
		...
		if (hit)
		{
			...
			b2Vec2 point =
			    (1.0f - fraction) * input.p1 + fraction * input.p2;
			return callback->ReportFixture(fixture, point,
						       output.normal, fraction);
		}
			...
			const b2Color &color)
{
	...
	case b2Shape::e_chain:
	{
		...
		b2Color ghostColor(0.75f * color.r, 0.75f * color.g,
				   0.75f * color.b, color.a);
...
void b2World::DrawDebugData()
{
	...
	if (flags & b2Draw::e_shapeBit)
	{
		for (b2Body *b = m_bodyList; b; b = b->GetNext())
		{
			...
			for (b2Fixture *f = b->GetFixtureList(); f;
			     f = f->GetNext())
			{
				if (b->IsActive() == false)
				{
				...
				{
					DrawShape(f, xf,
						  b2Color(0.5f, 0.9f, 0.5f));
				}
				else if (b->GetType() == b2_kinematicBody)
				{
					DrawShape(f, xf,
						  b2Color(0.5f, 0.5f, 0.9f));
				}
				else if (b->IsAwake() == false)
				{
					DrawShape(f, xf,
						  b2Color(0.6f, 0.6f, 0.6f));
				}
				else
				{
					DrawShape(f, xf,
						  b2Color(0.9f, 0.7f, 0.7f));
				}
			}
		}
	}
			     ...
			     f = f->GetNext())
			{
				for (int32 i = 0; i < f->m_proxyCount; ++i)
				{
					b2FixtureProxy *proxy =
					    f->m_proxies + i;
					b2AABB aabb =
					    bp->GetFatAABB(proxy->proxyId);
					b2Vec2 vs[4];
					vs[0].Set(aabb.lowerBound.x,
						  aabb.lowerBound.y);
					vs[1].Set(aabb.upperBound.x,
						  aabb.lowerBound.y);
					vs[2].Set(aabb.upperBound.x,
						  aabb.upperBound.y);
					vs[3].Set(aabb.lowerBound.x,
						  aabb.upperBound.y);
