...
class b2Shape
{
	...
	virtual bool TestPoint(const b2Transform &xf,
			       const b2Vec2 &p) const = 0;
	...
	virtual bool RayCast(b2RayCastOutput *output,
	...
	virtual void ComputeMass(b2MassData *massData,
				 float32 density) const = 0;

	Type m_type;

	/// Radius of a shape. For polygonal shapes this must be
	/// b2_polygonRadius. There is no support for making rounded polygons.
	float32 m_radius;
};
