...
void b2DistanceProxy::Set(const b2Shape *shape, int32 index)
{
	...
	{
		const b2ChainShape *chain =
		    static_cast<const b2ChainShape *>(shape);
	...
	case b2Shape::e_edge:
	{
		const b2EdgeShape *edge =
		    static_cast<const b2EdgeShape *>(shape);
...
struct b2Simplex
{
		       ...
		       const b2Transform &transformB)
	{
		...
		// different than old metric then flush the simplex.
		if (m_count > 1)
		{
			float32 metric1 = cache->metric;
			float32 metric2 = GetMetric();
			if (metric2 < 0.5f * metric1 ||
			    2.0f * metric1 < metric2 || metric2 < b2_epsilon)
			{
				// Reset the simplex.
				m_count = 0;
			}
		...
		const b2DistanceInput *input)
{
	...
	while (iter < k_maxIters)
	{
		...
		if (d.LengthSquared() < b2_epsilon * b2_epsilon)
		{
			...
			// We can't return zero here even though there may be
			// overlap. In case the simplex is a point, segment, or
			// triangle it is difficult to determine if the origin
		...
		vertex->wA =
		    b2Mul(transformA, proxyA->GetVertex(vertex->indexA));
		b2Vec2 wBLocal;
		vertex->indexB = proxyB->GetSupport(b2MulT(transformB.q, d));
		vertex->wB =
		    b2Mul(transformB, proxyB->GetVertex(vertex->indexB));
		vertex->w = vertex->wB - vertex->wA;

		// Iteration count is equated to the number of support point
		// calls.
		++iter;
		++b2_gjkIters;

		// Check for duplicate support points. This is the main
...
bool b2ShapeCast(b2ShapeCastOutput *output, const b2ShapeCastInput *input)
{
	...
	while (iter < k_maxIters && b2Abs(v.Length() - sigma) > tolerance)
	{
		...
		// Shift by lambda * r because we want the closest point to the
		// current clip point. Note that the support point p is not
		// shifted because we want the plane equation to be formed in
		// unshifted space.
		b2SimplexVertex *vertex = vertices + simplex.m_count;
